...
namespace DAL.Classes
{
    public class DBHelper
    {
        ...
        public static void llenarItems(ComboBox cb1, ComboBox cb2, ComboBox cb3)
        {
            SqlConnection cn = new SqlConnection(ConfigurationManager.AppSettings.Get("connectionString"));
            cn.Open();
            SqlCommand cmd = new SqlCommand("Select distinct Departamento from dbo.[Hurto-de-motocicletas-2016_bl] order by Departamento", cn);
            SqlDataReader dr = cmd.ExecuteReader();
            while(dr.Read())
            {
                cb1.Items.Add(dr["Departamento"].ToString());
            }
            dr.Close();
            cmd = new SqlCommand("Select distinct Municipio from dbo.[Hurto-de-motocicletas-2016_bl] order by Municipio", cn);
            dr = cmd.ExecuteReader();
            while (dr.Read())
            {
                cb2.Items.Add(dr["Municipio"].ToString());
            }
            dr.Close();
            cmd = new SqlCommand("Select distinct MARCA from dbo.[Hurto-de-motocicletas-2016_bl] order by MARCA", cn);
            dr = cmd.ExecuteReader();
            while (dr.Read())
            {
                cb3.Items.Add(dr["MARCA"].ToString());
            }
            dr.Close();
        }
